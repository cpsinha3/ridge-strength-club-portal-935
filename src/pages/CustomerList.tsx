...
export default function CustomerList() {
  ...
  const handleExport = () => {
    const headers = ['SFDC ID', 'Name', 'Relation', 'Relation Name', 'Mobile', 'Loan ID', 'EMI Amount', 'Tenure', 'Disb. Amount', 'B. Loan Amount', 'Disb. Date', 'UTR Number', 'Engine Number', 'Frame Number', 'Start Month', 'End Month', 'Sales Point', 'AT', 'PO', 'PS', 'DIST', 'LANDMARK', 'STATE', 'PIN', 'DOB', 'Adhar Number', 'Notes'];
    const csvContent = [
      headers.join(','),
          ...
          <table className="w-full text-sm">
            <thead className="bg-carbon-lighter border-b border-border">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-chalk">Engine Number</th>
                <th className="px-4 py-3 text-left font-medium text-chalk">Frame Number</th>
              ...
              {filteredCustomers.length === 0 ? (
                ...
                filteredCustomers.map((customer) => (
                  <tr key={customer._row_id} className="hover:bg-carbon-lighter/50">
                    ...
                    <td className="px-4 py-3 text-chalk font-mono text-xs">{customer.engine_number || '-'}</td>
                    <td className="px-4 py-3 text-chalk font-mono text-xs">{customer.frame_number || '-'}</td>
